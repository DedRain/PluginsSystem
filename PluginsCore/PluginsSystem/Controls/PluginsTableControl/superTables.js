...
var superTable = function (tableId, options) {
    /////* Initialize */
    var options = { cssSkin: "sSky", headerRows: "1", fixedCols: "0" };
    ...
    for (var i = 1; i < this.sDataTable.rows.length; ++i) {
        var row = this.sDataTable.rows[i];

        var cell = row.cells[0];
        var control = cell.childNodes[0];
        $(control).click(function (event) {
            var checkBox = this;
            stateControlClick.call(checkBox);
        });
        //control.onclick = function (event) { stateControlClick.call(control); };
    }
};

function stateControlClick() {
    var url = "/SavePluginState.aspx?";
    url += "ID=" + this.parentElement.parentElement.attributes["PluginID"].value + "&";
    url += "State=" + this.checked;
